...
// app.header("Access-control-allow-origin", "localhost:5173");
...
app.delete("/tasks", async (req, res) => {
  await tasks.deleteMany({ done: true });
  return res.sendStatus(204);
});
...
app.put("/tasks/:id", async (req, res) => {
  ...
  try {
    ...
    await tasks.updateOne({ _id }, { $set: { subject } });
    const data = await tasks.findOne({ _id });
...
app.put("/tasks/toggle/:id", async (req, res) => {
  ...
  //   { _id },
  //   { $set: { done: current.done ? false : true } }
  // );

  await tasks.updateOne({ _id }, [
